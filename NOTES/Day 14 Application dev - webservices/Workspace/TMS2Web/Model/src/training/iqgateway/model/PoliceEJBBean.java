...
@Stateless(name = "PoliceEJB", mappedName = "TMSWebApplication-Model-PoliceEJB")
public class PoliceEJBBean implements PoliceEJB, PoliceEJBLocal {
    public PoliceEJBBean() {
    }
    
    
    AdminDAO plcOpRef = new AdminDAOImpl();
    ...
    public List<OffenceTypesEO> findAllOffenceType (){
        List<OffenceTypesEO> offenceEOList = OffTypesOpRef.findAllOffenceTypes();
        return offenceEOList;
    }
    
    public List<OffenceTypesEO> findOffenceByVehicleType (String vehicleType){
        List<OffenceTypesEO> offenceEOList = OffTypesOpRef.findOffenceTypesByVehicleType(vehicleType);
        return offenceEOList;
    }
    
    public List<OffenceTypesEO> findOffenceByOffenceType (String offenceType){
        List<OffenceTypesEO> offenceEOList = OffTypesOpRef.findOffenceTypesByOffenceType(offenceType);
        return offenceEOList;
    }
    ...
    public String addVehicleOffence (VehicleOffenceEO vehOffEO){
        if(vehOffEO.getVehicle_offence_id() == null || vehOffEO.getOffence_id() == null ||
            vehOffEO.getLocation() == null || vehOffEO.getRegistration_id() == null ||
           vehOffEO.getProof2() == null){
               return "Error: Incomplete fields";
        }
        List<VehicleOffenceEO> vehOffEOList = vehOffOpRef.findVehicleOffenceByRegisID(vehOffEO.getRegistration_id().getRegistration_id());
