...
@Stateless(name = "AdminEJB", mappedName = "TMSWebApplication-Model-AdminEJB")
public class AdminEJBBean implements AdminEJB, AdminEJBLocal {
    public AdminEJBBean() {
    }
    
    AdminDAO adminOpRef = new AdminDAOImpl();
    RoleDAO roleOpRef = new RoleDAOImpl();

    public String addAdmin(AdminEO adminEO) {
        
        if (adminEO.getRole_id() == null || adminEO.getName() == null ||
    ...
    public String addRole (RoleEO roleEO){
        if(roleEO.getRole_name() == null) return "Error : Incomplete fields";
        List<RoleEO> role = new ArrayList<RoleEO>();
        role = roleOpRef.findRoleByRoleName(roleEO.getRole_name());             
