...
public class AdminServiceImpl implements AdminService {
	...
	public AdminEO persistAdminEO(AdminEO adminEO) {
		// Fetch max id and increment by 1
	...
	public List<AdminEO> getAdminEOFindAll() {
		List<AdminEO> adminEOList = new ArrayList<AdminEO>();
		adminRepositoryRef.findAll().forEach(adminEOList::add);
		return adminEOList;
	}
	...
	public List<AdminEO> findAdminByRoleID(Long roleId) {
		return adminRepositoryRef.findByRoleId(roleId);
	}
	...
	public List<RoleEO> getRoleEOFindAll() {
		List<RoleEO> roleEOList = new ArrayList<RoleEO>();
		rolesRepositoryRef.findAll().forEach(roleEOList::add);
		return roleEOList;
	}
	...
	// public RoleEO findById(Long roleId) {
	// return rolesRepositoryRef.findById(roleId).orElse(null);
	// }
