...
public class LoginServlet extends HttpServlet {
    ...
    protected boolean verifyPassword(String user, String pwd){
        String originalPassword = null;
        try{
            con = getConnection();
            Statement stmt = con.createStatement();
            stmt.executeQuery("SELECT password FROM login where uname = '" + user + "'");
            ResultSet rs = stmt.getResultSet();
            if(rs.next()){
                originalPassword = rs.getString(1);
            }
            stmt.close();
            if(originalPassword.equals(pwd)){
                return true;
            }else{
                return false;
            }
...
//            Statement stmt = con.createStatement();
//
//            stmt.executeQuery("select password from login where uname='" +
//                              theuser + "'");
//
//            ResultSet rs = stmt.getResultSet();
//
...
//        } catch (Exception e) {
//
//            System.out.println("Exception in verifyPassword()=" +
//                               e.toString());
//
//            return false;
//
...
//                        ":Your login module is working great!");
//
//        } else {
//
//            out.println("Invalid user name or password");
