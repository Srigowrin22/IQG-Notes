...
import training.iqgateway.operations.ClerkOperation;

public class ClerkOperationImpl implements ClerkOperation{
    
    AdminDAO clkOpRef = GenericDAOFactory.createAdminDAO();
    ...
    public String addVehicleOffence (VehicleOffenceEO vehOffEO){
        if(vehOffEO.getOffenceID() == null ||
            vehOffEO.getLocation() == null ||
            vehOffEO.getRegistrationID() == null){
...
//        if (user.getName().equals(adminEO.getName()) &&
//            user.getPassword().equals(adminEO.getPassword())) {
//            int authorize = CLKOpRef.authorizeAdmin(adminEO.getDesignationID(), adminEO.getPassword());
